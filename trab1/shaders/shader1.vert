...
void main() 
{
    ...
    if(distance(totalDirections.x, totalDirections.y)> 0.25) {
        totalDirections = normalDirection * 0.2;
    }

    gl_PointSize = size;
    color = colorize(vec2(totalDirections.x, totalDirections.y));
    gl_Position = vec4((vPosition.x + totalDirections.x)/table_dim_width, 
    (vPosition.y + totalDirections.y)/table_dim_height, 0.0, 1.0);
