...
function setup(shaders)
{
    ...
    canvas.addEventListener("click", function(event) {
        ...
        if(upOrDawn) {
            gl.bufferSubData(gl.ARRAY_BUFFER, vertices.length * MV.sizeof["vec3"] + 
                    (protatingz.length % MAX) * MV.sizeof["vec3"], MV.flatten(MV.vec3(x,y, 1)));
            protatingz.push(MV.vec3(x, y, 1));
            console.log("pos");
        } else {
            gl.bufferSubData(gl.ARRAY_BUFFER, vertices.length * MV.sizeof["vec3"] + 
                    MAX * MV.sizeof["vec3"] + (eletratingz.length % MAX) * MV.sizeof["vec3"], MV.flatten(MV.vec3(x,y, -1)));
